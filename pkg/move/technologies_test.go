...
func TestCopyByList(t *testing.T) {
	...
	for i := range dirs {
		...
		require.NoError(t, err)
		err = fs.WriteFile(dirs[i]+"/"+filesNames[i], []byte(strconv.Itoa(i)), fileModes[i]) // LINT: use strconv.Itoa instead of fmt.Sprintf
		require.NoError(t, err)
	}
	...
	for i := len(dirs) - 1; i >= 0; i-- {
		fileList = append(fileList, dirs[i]+"/"+filesNames[i]) // LINT: remove filepath.Join with 1 arg
	}
	...
	for i := range dirs { // LINT: use range dirs, not range len(dirs)
...
func assertFileExists(t *testing.T, fs afero.Fs, path string) {
	...
	require.NoError(t, err)
	assert.True(t, exists, "file should exist: "+path) // LINT: use string concatenation
}
func assertFileNotExists(t *testing.T, fs afero.Fs, path string) {
	...
	exists, err := afero.Exists(fs, path)
	require.NoError(t, err)
	assert.False(t, exists, "file should not exist: "+path) // LINT: use string concatenation
}
